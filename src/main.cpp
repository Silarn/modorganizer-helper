...
static bool createMODirectory(const QString& directoryName, const std::wstring& accountName) {
    if (!QDir(directoryName).exists()) {
        if (!QDir().mkdir(directoryName)) {
            qCritical("Failed to create \"%s\", do you have the necessary access rights to the installation folder?",
...
static bool init(const QString& mopath, const std::wstring& accountName) {
    if (!SetOwner(ToWString(mopath).c_str(), accountName.c_str())) {
        qCritical("failed to set owner of \"%s\" to \"%ls\"", mopath.toUtf8().constData(), accountName.c_str());
        return false;
    }
...
static bool backdateBSAs(const QString& dataPath) {
    ...
    while (iter.hasNext()) {
        ...
        HANDLE file = ::CreateFileW(ToWString(iter.filePath()).c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr,
        ...
        if (!::SetFileTime(file, nullptr, nullptr, &newWriteTime)) {
            qCritical("failed to change date for %s: errorcode %d", iter.filePath().toUtf8().constData(),
                      ::GetLastError());
            return false;
        }
