...
BOOL SetPrivilege(HANDLE token, LPCWSTR privilege, BOOL enable) {
    ...
    if (!LookupPrivilegeValue(nullptr, privilege, &luid)) {
        qCritical("failed to look up privilege %ls: %lu", privilege, ::GetLastError());
        return FALSE;
    }
    ...
    if (!AdjustTokenPrivileges(token, FALSE, &tokenPriv, sizeof(TOKEN_PRIVILEGES), (PTOKEN_PRIVILEGES) nullptr,
                               (PDWORD) nullptr)) {
        qCritical("failed to adjust privilege %ls: %lu", privilege, ::GetLastError());
        return FALSE;
    }

    if (::GetLastError() == ERROR_NOT_ALL_ASSIGNED) {
        qCritical("The token does not have the specified privilege %s.", privilege);
        return FALSE;
    }
...
BOOL SetOwner(LPCTSTR filename, LPCTSTR newOwner) {
    ...
    if (!LookupAccountName(nullptr, newOwner, sid, &sidSize, domainUnused, &domainBufSize, &sidUse)) {
        ...
        secRes =
            SetNamedSecurityInfo(fileNameBuf, SE_FILE_OBJECT, OWNER_SECURITY_INFORMATION | DACL_SECURITY_INFORMATION,
